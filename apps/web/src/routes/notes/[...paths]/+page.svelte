...
<div class="relative mx-auto max-w-prose">
  ...
  <main>
    ...
    {:else}
      <div
        class="font-inter prose prose-invert prose-headings:before:-translate-x-2 prose-headings:before:absolute prose-headings:before:font-normal prose-headings:before:right-full prose-headings:before:text-zinc-700 prose-headings:relative prose-headings:font-normal prose-h1:before:content-['H1.'] prose-h2:before:content-['H2.'] prose-h3:before:content-['H3.'] prose-h4:font-semibold prose-p:my-8 prose-p:after:hidden prose-p:before:hidden prose-a:font-normal hover:prose-a:text-rose-500 hover:prose-a:underline prose-a:no-underline prose-a:text-rose-600 target-blank:[&:not(:has(img))]:prose-a:after:content-[url('/images/external-link.svg')] target-blank:[&:not(:has(img))]:prose-a:after:mx-1 prose-img:my-1.5 prose-li:my-1.5 prose-pre:bg-zinc-900 prose-code:after:hidden prose-code:before:hidden prose-code:bg-zinc-700 prose-code:font-normal prose-code:mx-0.5 prose-code:px-1 prose-code:py-0.5 prose-code:rounded leading-[1.85]"
      >
        {@html content}
      </div>
    {/if}
